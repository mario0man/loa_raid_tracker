...
from uwu7 import print_raid_logs, fetch_page, extract_roster, extract_logs, _fetch_boss_to_raid, get_last_weekly_reset, _format_ilvl, _register_explicit_raids
...
def get_character_logs(char_name, boss_to_raid):
    from uwu7 import _fetch_character_logs
...
def build_table(main_character, roster=None, _skip_no_logs=False):
    ...
    # Performance percentile per gate cleared THIS week, taken from that gate's
    # own log entry (0-1 float + which metric it is: "contribution" for
    # supports, "damage" for DPS). Gates marked ✅ via the high-water-mark rule
    # (no log entry of their own this week) and entries whose percentile is
    # null stay out of this map — those keep the plain ✅ in the table.
    char_raid_gate_pct = {}  # (character, raid, gate) -> (percentile, percentile_kind)
    for entry in all_entries:
        if entry["completion_dt"] >= reset_time and entry["percentile"] is not None:
            key = (entry["character"], entry["raid_name"], entry["gate_number"])
            char_raid_gate_pct[key] = (entry["percentile"], entry["percentile_kind"])
    ...
    return {
        ...
        "char_raid_gate_pct": char_raid_gate_pct,
...
if st.session_state.get("accent_data"):
    ...
    if data:
        ...
        gate_pct = data["char_raid_gate_pct"]

        def _gate_icon(status, gate, pct=None, pct_kind=None):
            """Render a single gate's number + status marker with a hover tooltip.

            A gate cleared this week with a non-null performance percentile
            shows that percentile as a green number instead of the ✅ emoji —
            the same value lostark.bible badges for the log (damage percentile
            for DPS, contribution percentile for supports). A null percentile
            or a gate marked cleared without its own log entry keeps the ✅.
            """
            if status == "✅" and pct is not None:
                n = round(pct * 100)
                kind_txt = " (contribution)" if pct_kind == "contribution" else ""
                label = f"Gate {gate}: cleared this week — {n}th percentile{kind_txt}"
                return (
                    f"<span title='{label}' "
        ...
        def _gate_cell(char_name, raid):
            ...
            for gate in range(1, total + 1):
                ...
                pct, pct_kind = gate_pct.get((char_name, raid, gate), (None, None))
                icons.append(_gate_icon(status, gate, pct, pct_kind))
            return "".join(icons)
        ...
        st.markdown(
            "<span style='color:#4caf50; font-weight:600;'>95</span> Gate cleared this week — percentile &nbsp;&nbsp; "
            "<span style='color:#4caf50'>✅</span> Gate cleared this week (no percentile logged) &nbsp;&nbsp; "
