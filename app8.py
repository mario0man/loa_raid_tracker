...
from uwu8 import print_raid_logs, fetch_page, extract_roster, extract_logs, _fetch_boss_to_raid, get_last_weekly_reset, _format_ilvl, _register_explicit_raids
...
def get_character_logs(char_name, boss_to_raid):
    from uwu8 import _fetch_character_logs
...
def build_table(main_character, roster=None, _skip_no_logs=False):
    ...
    def _window_view(window_start, window_end=None):
        ...
        gate_pct = {}  # (character, raid, gate) -> (percentile, percentile_kind)
        for entry in all_entries:
            dt = entry["completion_dt"]
            if (
                entry["percentile"] is not None
                and dt >= window_start
                and (window_end is None or dt < window_end)
            ):
                key = (entry["character"], entry["raid_name"], entry["gate_number"])
                # The site lists newest first; keep the newest in-window value.
                gate_pct.setdefault(key, (entry["percentile"], entry["percentile_kind"]))
        ...
        return {"char_raid_gate_status": gate_status, "char_raid_gate_pct": gate_pct, "completed": completed}
